...
<html lang="en">
    ...
    <body>
        ...
        <table class="info-table">
            <tr>
                <td class="w-50 text-left align-center">
                    <div class="mb-6">
        ...
        <table class="excel-table">
            <thead>
                <tr class="thead-bg">
                    ...
                    <th class="col-tgl">Tanggal Penerimaan</th>
                    <th class="col-berat">Supplier</th>
                    <th class="col-suhu">Grade/Sizing</th>
            ...
            <tbody>
                @forelse (($data ?? []) as $row)
                    <tr>
                        ...
                        <td>
                            {{ $row->grade?->grade ?? '-' }} -
                            {{ $row->kategoriBeratPenerimaan?->kategori_berat ?? '-' }}
                        </td>
                        <td>{{ $row->total_berat_ikan ?? '-' }}</td>
                        <td>{{ $row->jumlah_penerimaan ?? '-' }}</td>
                    </tr>
                @empty
                    <tr>
            ...
            @if (!empty($data) && count($data) > 0)
                <tfoot>
                    <tr class="total-row">
                        ...
                        <td colspan="3"></td>
                        <td>
                            {{ number_format($data->sum('total_berat_ikan') ?? 0, 2) }} kg
                        </td>
                        <td>
                            {{ $data->sum('jumlah_penerimaan') }} ekor
                        </td>
                    </tr>
                </tfoot>
            @endif
